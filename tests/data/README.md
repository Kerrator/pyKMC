...
| Event | pARTn-found symmetric vacancy migration; moving atom `central_atom_idx = 1033`; `dE_forward = 0.50889 eV`, `dE_backward = 0.50889 eV` (NEB-validated on the originating line); all positions inside `[0, L)` |
...
| Origin | `event_geometry_output` dump of the originating HTST line (commit `c7acf50`), tracked unchanged |
...
The forward value reproduces the donor line's 23.6 THz / 43 free atoms
(`33fa4a2`). The FD kernel (`pykmc.htst.fd_hessian_fn`) on the same engine
