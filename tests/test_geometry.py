...
def test_push_towards_mixed_pbc_only_wraps_periodic_dims() -> None:
    """push_towards must minimum-image only the periodic directions."""
    from pykmc.utils.geometry import push_towards

    current = np.array([[1.0, 1.0, 1.0]])
    target = np.array([[9.5, 9.5, 1.0]])

    # x periodic, y non-periodic: x wraps toward the near image, y does not.
    got = push_towards(
        current.copy(),
        target.copy(),
        fraction=0.5,
    ...
    # x is periodic in both -> identical; the non-periodic y differs.
    assert got[0, 0] == pytest.approx(ref[0, 0])
    assert got[0, 1] != pytest.approx(ref[0, 1])
...
def test_push_towards_scalar_pbc_does_not_crash() -> None:
    ...
    for scalar, vector in (
        ...
        got = push_towards(
            current.copy(), target.copy(), fraction=0.5, cell=CELL, pbc=scalar
        )
        ref = push_towards(
            current.copy(), target.copy(), fraction=0.5, cell=CELL, pbc=vector
        )
