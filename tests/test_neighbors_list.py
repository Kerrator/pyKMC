...
def test_mixed_pbc_rcut_includes_central_atom() -> None:
    ...
    """
    nl_mixed = NeighborsList(
        _simple_cubic_system(np.array([True, True, False])), 1.1, 1.8
    )
    nl_periodic = NeighborsList(
        _simple_cubic_system(np.array([True, True, True])), 1.1, 1.8
    )
