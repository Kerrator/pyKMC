...
#   B at A + (8, 0, 0)   -> ~8 A from A   (must NOT be recycled)
#   C at A + (14, 14, 5) -> ~20.4 A from A (must be recycled)
#
# Box L = 35.2 A limits single-axis PBC distance to L/2 = 17.6 A, so "far"
...
def make_ni_fcc_with_vacancies(
    ...
    (system, vacancy_post_indices, central_atom_indices)
        - system : 4000 - len(vacancy_targets) atoms, box L = 35.2 A, cubic.
...
def ni_fcc_3vacancies() -> tuple[System, list[int]]:
    """Ni FCC supercell with vacancies at A (center), B (~8 A), C (~20 A)."""
...
def ni_fcc_4vacancies() -> tuple[System, list[int]]:
    ...
    minimum-image distance from A is ~2.2 A. Confirms PBC awareness.
    """
