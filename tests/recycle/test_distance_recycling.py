...
Three vacancies are placed so that two events are close together (A & B, ~8 A
apart) and a third is far (C, ~20 A on a diagonal). Executing the event at
vacancy A should:
...
def test_close_discarded_far_recycled(ni_fcc_3vacancies) -> None:
    """8-A vacancy must NOT be recycled; 20-A vacancy MUST be recycled."""
    system, central = ni_fcc_3vacancies
    assert len(system.positions) == 4000 - 3
    positions_pre = system.positions.copy()
    # Simulate execution at vacancy A: shift its central atom by 0.3 A.
...
def test_distance_threshold_boundary(ni_fcc_3vacancies) -> None:
    """Widen distance_thr past C's ~20 A -> C is no longer 'far' -> 0 recycled."""
    system, central = ni_fcc_3vacancies
    positions_pre = system.positions.copy()
    system.positions[central[0]] = positions_pre[central[0]] + np.array([0.3, 0.0, 0.0])
    active = make_active_table([row(c) for c in central])
...
def test_movement_check_overrides_distance(ni_fcc_3vacancies) -> None:
    """Move C's central atom by 0.05 A (> movement_thr) -> not recycled despite being far."""
    system, central = ni_fcc_3vacancies
    positions_pre = system.positions.copy()
    system.positions[central[0]] = positions_pre[central[0]] + np.array([0.3, 0.0, 0.0])
...
def test_pbc_wrap_close(ni_fcc_4vacancies) -> None:
    ...
    With L=35.2, D at A + (33,0,0) wraps to A - (2.2,0,0) -> PBC distance ~ 2.2 A.
    Only the 20-A vacancy C should be recycled.
    """
