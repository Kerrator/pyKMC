...
The frozen R09/N06 ``changed`` case at unit level: with ``free_radius`` 0.25 Å
...
from . import test_site_dependencies as d
...
def geometry(a: float, *, remote_shift: float = 0.0) -> np.ndarray:
    """Frozen R09 LJ6 case: the centre, four anchors at r0 and a remote atom at 20 Å."""
...
def site_state(settings: HTSTSettings | None = None, **kwargs) -> SiteState:
    ...
    # The crop is the centre alone, as in the frozen fixture (rcut = 0.25 Å).
    signature = (
        SOURCE_IDS[0],
        REF,
        (SOURCE_IDS[0],),
...
def test_motion_beyond_the_interaction_shell_keeps_the_site_row() -> None:
    """R09 ``same`` control: only the remote executed centre, 20 Å away, moves."""
    state = site_state()
...
def test_validate_recycled_drops_the_row_whose_shell_atom_moved() -> None:
    """Through the table: a moved atom outside ``free_radius`` but inside the range."""
    cfg, system, _, _ = h.setup()
    # Atom 2 sits 21 Å from the centre: outside free_radius 6 Å and the crop,
    # inside 6 + 16 Å. It is the executed atom of the recycling fixtures.
    cfg = cfg.model_copy(
        update={
            "rateconstant": cfg.rateconstant.model_copy(
                update={"interaction_range": 16.0}
            )
        }
    )
    svc, worker = d.current_service(cfg, system)
    active, neighbors, summary = d.seed_site(cfg, system, svc)
    assert summary["ok"] == 1
    system.positions[2, 0] += 0.1
    assert active.validate_recycled(system, neighbors) == 1
    d.assert_dropped(active)
    assert len(worker.requests) == 1


def test_validate_recycled_keeps_the_row_when_the_moved_atom_is_beyond_the_range() -> (
    None
):
    """The default region does not reach the 21 Å executed atom of the fixtures."""
    cfg, system, _, _ = h.setup()
    rc = cfg.rateconstant
    assert rc.free_radius + rc.interaction_range < 21.0
    svc, worker = d.current_service(cfg, system)
    active, neighbors, summary = d.seed_site(cfg, system, svc)
    assert summary["ok"] == 1
    record = h.site_record(active, 0)
    system.positions[2, 0] += 0.1
    assert active.validate_recycled(system, neighbors) == 0
