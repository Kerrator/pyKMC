...
def test_htst_fallback_keeps_k0_and_logs(
    ...
    assert row["nu0"] is None or (
        isinstance(row["nu0"], float) and math.isnan(row["nu0"])
    )
...
def test_backfill_only_accepted_events(
    ...
    results = table.add_events([accepted, rejected, duplicate], types=list(sys_.types))
...
def test_backfill_patches_directional_rows(
    ...
    assert math.isclose(
        fwd_row["k"], table.rate_constant.compute_rate(0.5, nu0=5.0e12).rate
    )
    ...
    assert math.isclose(
        bwd_row["k"], table.rate_constant.compute_rate(0.7, nu0=3.0e12).rate
    )
