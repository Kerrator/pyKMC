...
def test_active_event_k_uses_inherited_k_prefactor(
    ...
    series = table.build_event_series(
        _refinement_output(k_prefactor=5.0e12, nu0=5.0e12)
    )
    assert math.isclose(
        series["k"], rate_from_prefactor(5.0e12, 0.5, config.rateconstant.T)
    )
...
def test_active_event_k_uses_k0_prefactor_on_fallback(
    ...
    assert math.isclose(
        series["k"], rate_from_prefactor(k0, 0.5, config.rateconstant.T)
    )
