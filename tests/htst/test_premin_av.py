...
def _htst(engine: object) -> HtstLammpsExtension:
    """The extension instance attached by ``build_serial_engine`` (private helpers)."""
    return engine._extensions[HtstLammpsExtension.__name__]  # type: ignore[attr-defined]
...
def test_premin_surroundings_relaxes() -> None:
    ...
    e_before = engine.get_total_energy(positions=rattled.copy())
    relaxed = _htst(engine)._premin_surroundings(cfg, rattled.copy(), move)
    e_after = engine.get_total_energy(positions=relaxed.copy())
...
def test_premin_recovers_rattled_environment() -> None:
    ...
    ref = engine.compute_event_prefactors(
        _Cfg(premin=False),
        min1_positions=init,
    ...
    raw = engine.compute_event_prefactors(
        _Cfg(premin=False),
        min1_positions=r_init,
        saddle_positions=r_sad,
        min2_positions=r_fin,
        **kwargs,
    ).nu0_forward
    healed = engine.compute_event_prefactors(
        _Cfg(premin=True),
        min1_positions=r_init,
        saddle_positions=r_sad,
        min2_positions=r_fin,
...
def test_premin_false_is_deterministic_old_path() -> None:
    ...
    res1 = engine.compute_event_prefactors(
        cfg,
    ...
    res2 = engine.compute_event_prefactors(
        cfg,
        central_atom_idx=move,
        min1_positions=init,
        saddle_positions=sad,
...
def test_active_volume_branch_returns_prefactors() -> None:
    ...
    res = engine.compute_event_prefactors(
        _Cfg(premin=False, active_volume=True, ract=6.0),
        central_atom_idx=move,
        min1_positions=init,
        saddle_positions=sad,
        min2_positions=fin,
...
def test_active_volume_ract_guard() -> None:
    ...
    res = engine.compute_event_prefactors(
        _Cfg(premin=False, active_volume=True, ract=3.0),
        central_atom_idx=move,
        min1_positions=init,
        saddle_positions=sad,
        min2_positions=fin,
