...
def setup(monkeypatch, outcome, active_volume=True):
    ...
    def prepare(engine, _cfg, center, positions, cell, types):
        assert center == 0
        np.testing.assert_array_equal(positions, POSITIONS)
    ...
    def prepare_refine(engine, cfg, center, positions, cell, types, idx, saddle):
        mapping, central = prepare(engine, cfg, center, positions, cell, types)
