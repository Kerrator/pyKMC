...
class TestSolver : 
 ...
 [-2.83934789e-10,-2.83934789e-10,-2.83934789e-10, 0.00000000e+00]])
    ...
    def test_qsd_solver_stiff_system(self, test_logger):
        ...
        # Transient rates ~1e-2, absorbing rates ~1e-12 => stiffness ~2e10.
        # The diagonal is the TOTAL out-rate (mixing + escape): the MFPT solver inverts
        # the transient block, which is exactly singular if the escape is left out of it.
        # The escape must also stay representable next to the mixing rate -- at the
        # original 1e-18 the sum 2e-2 + 1e-18 rounds back to 2e-2 in float64 and the
        # block is singular again whatever is written.
        g = 1.00e-12
        M = np.array([
            [+2.00e-02 + g, -1.00e-02,     -1.00e-02,     0.0],
            [-1.00e-02,     +2.00e-02 + g, -1.00e-02,     0.0],
            [-1.00e-02,     -1.00e-02,     +2.00e-02 + g, 0.0],
            [-g,            -g,            -g,            0.0],
        ])
    ...
    def test_qsd_matches_bisection_moderate(self, test_logger):
        ...
        # The diagonal is the TOTAL out-rate: mixing + escape. Leaving the escape out
        # makes the transient block exactly singular, which the MFPT solver inverts.
        w, g = 9.48225009e-03, 2.83934789e-10
        M = np.array([
            [+2 * w + g, -w,         -w,         0.0],
            [-w,         +2 * w + g, -w,         0.0],
            [-w,         -w,         +2 * w + g, 0.0],
