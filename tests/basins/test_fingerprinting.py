...
class TestFingerprinting:
    ...
    def test_com_fingerprint_boundary_crossing_invariance(self):
        ...
        shifted = positions + np.array([9.7, 9.7, 9.7])  # 0.5 + 9.7 = 10.2 -> wraps to 0.2
    ...
    def test_aoi_fingerprint_boundary_crossing_invariance(self):
        ...
        cell = np.diag([10.0, 10.0, 10.0])
        pbc = np.array([True, True, True])
        fp1 = fp.atoms_of_interest_fingerprint(positions, cell, pbc, rnei=1.5, coord_thr=10)
    ...
    def test_circular_mean_localized_cluster(self):
        ...
        assert np.allclose(_mic_dists(cluster, com1), _mic_dists(shifted, com2), atol=1e-10)
