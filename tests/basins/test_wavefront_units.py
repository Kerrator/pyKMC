...
class TestIsNewStateBatch:
    def test_detects_existing_intra_batch_and_new(self):
        ...
        sys_b = _make_system(
            np.array(
                [[5.0, 5.0, 5.0], [6.5, 5.0, 5.0], [5.0, 6.5, 5.0], [5.0, 5.0, 6.5]]
            )
        )
...
class TestResultFromMpi:
    def test_engine_positions_sanitized_into_box(self):
        ...
        config = _com_config()
        basin = _make_basin(config)
        from_positions = np.array(
            [[0.0, 0.0, 0.0], [2.0, 2.0, 0.0], [1.0, 1.0, 1.5], [3.0, 3.0, 1.5]]
        )
        basin.states[0] = StateData(
            system=_make_system(from_positions), environment=None, neighbors_list=None
        )
    ...
    def test_error_payload_maps_to_err(self):
        basin = _make_basin(_com_config())
        result = basin._result_from_mpi(
            {"ok": False, "error_type": "PSR_NO_MATCH_FOUND", "message": "no match"},
            from_state=0,
        )
...
class TestSerialTimingCheckpoint:
    def test_finalizer_writes_serial_checkpoints(self, tmp_path, monkeypatch):
        ...
        result = basin._finalize_exploration_run(
            _time.perf_counter() - 1.0, prof, 3, 1, strategy_label="serial"
        )
        assert result.is_ok()
...
class TestCapRemainingAsAbsorbing:
    def test_materialized_frontier_flips_to_absorbing(self):
        ...
        for idx in (0, 1, 2):
            basin.states[idx] = StateData(
                system=_make_system(pos + idx),
                environment=None,
                neighbors_list=None,
                transient=True,
            )
...
class TestMarkFailedAbsorbing:
    def test_row_kept_absorbing_and_excluded_from_exit_draw(self):
        ...
        err = ErrorInfo(
            type=ErrorType.RECONSTRUCTION_INVALID_MIN2, message="delr2 = 2.46"
        )
...
class TestMaterializeExitState:
    def test_deferred_system_is_deduped_and_merged(self):
        ...
        basin.states[1] = StateData(
            system=_make_system(pos + 3.0),
            environment=None,
            neighbors_list=None,
            transient=False,
        )
        basin._state_fingerprints[1] = fingerprinting.compute_fingerprint(
            config,
            basin.states[1].system.positions,
            basin.states[1].system.cell,
            basin.states[1].system.pbc,
        )
        ...
        basin.absorbing_saddle_positions[basin._saddle_key(0, 2, 0, 12, 0)] = np.array([[0.5, 0.5, 0.5]])
        basin.connectivity_table.add_connectivity(
            state=0,
            state_connexion=2,
            event_connexion=12,
        ...
        df = basin.connectivity_table.df
        assert (df["state_connexion"] == 1).any() and not (
            df["state_connexion"] == 2
        ).any()
        assert basin._saddle_key(0, 1, 0, 12, 0) in basin.absorbing_saddle_positions
...
class TestSkipRefinement:
    def test_fallback_saddle_keeps_row_selectable(self):
        ...
        n = basin._skip_refinement(
            idx_with, {"fallback_saddle": saddle}, "transport: boom", 0
        )
        n = basin._skip_refinement(idx_without, None, "PSR failed", n)

        assert n == 2
        assert np.allclose(basin.absorbing_saddle_positions[basin._saddle_key(0, 1, 0, 11, 0)], saddle)
...
class TestBudgetBreachReason:
    def test_each_knob_fires(self):
        ...
        assert (
            basin._budget_breach_reason(1, t_now - 1.0) == "max_basin_walltime_s=0.001"
        )
