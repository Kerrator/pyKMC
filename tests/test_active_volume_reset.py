...
``reset()`` used to hard-code ``create_box 1 box`` and emit no ``mass``: every
active-volume (AV) crop and every HTST nu0 zone rebuilt the LAMMPS instance as
a one-type, mass-less box. A multi-element ``pair_coeff`` (``eam/alloy ... Cr
Ni``) was rejected inside ``reset()``, and pair styles that do not set masses
themselves (``lj/cut``, ``sw``, ``mlip``) aborted at the first rebalance with
"Not all per-type masses are set" even for a single species. Inside the HTST
prefactor op that abort is swallowed, so every SW-Si event silently fell back
to ``k0``.
...
engine's integer types, and drive the actual ``partn_search_AV`` entry point on
Ni and NiCr cells under ``lj/cut``, on NiCr under the shipped NiFeCr
``eam/alloy`` potential, and on Si under ``sw``.
"""
...
class _SerialEngine:
    """One serial LAMMPS behind the ``.command`` / ``.lmp`` surface the ops use."""
    ...
    def command(self, cmd: str) -> None:
        """Run ``cmd`` on the LAMMPS instance."""
        self.lmp.command(cmd)

    def close(self) -> None:
        """Release the LAMMPS instance."""
        self.lmp.close()
...
def test_map_types_matches_lammps_engine_types() -> None:
    ...
    system = SimpleNamespace(types=types, positions=positions, cell=cell)
    engine = _SerialEngine()
    try:
        ops.initialize_parameters(engine)
        ops.initialize_system(engine, system)
        ops.initialize_potential(engine, _SearchCfg())
...
def test_partn_search_AV_crop_holds_every_species(
    ...
    rebalance aborts unless ``reset()`` emitted them.
    """
    _require_serial()
    if cell_kind == "Si":
