...
def event_contained(
    ...
    *not contained* (a reject), not silently skipped: the guard is the only
    ...
    pbc : bool or np.ndarray, optional
        Periodic axes; distances use the minimum image on those axes only.
    ...
    central_pos = np.asarray(min1_positions)[central_rows[0]]
    max_mover_r = 0.0
    for state in (min1_positions, saddle_positions, min2_positions):
        separations = minimum_image_displacement(
            np.asarray(state)[mv] - central_pos, cell, pbc
        )
        max_mover_r = max(max_mover_r, float(np.linalg.norm(separations, axis=1).max()))
