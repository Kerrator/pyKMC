    ...
    def ensure_full_state(self, config: Config) -> None : 
        if self.system is not None : 
            ...
            if self.environment is None :
                self.environment = AtomicEnvironment(config.atomicenvironment.style, self.neighbors_list.neighbors_list['rnei'], self.neighbors_list.neighbors_list['rcut'], config.atomicenvironment.neighbors_add, types=self.system.types, coloring_mode=config.atomicenvironment.atom_coloring_mode, coordination_threshold=config.atomicenvironment.coordination_threshold)
...
#        ref_event = self.reference_table.table.iloc[event_idx].copy()
    ...
    def _add_state(self, state_index, system=None, transient=True, applicable_events=None, visited=False, full=False ) :
        ...
        if full == True : 
            ...
            atomic_environment = AtomicEnvironment(self.config.atomicenvironment.style, neighbors_list.neighbors_list['rnei'], neighbors_list.neighbors_list['rcut'], self.config.atomicenvironment.neighbors_add, types=system.types, coloring_mode=self.config.atomicenvironment.atom_coloring_mode, coordination_threshold=self.config.atomicenvironment.coordination_threshold)
        else : 
            neighbors_list = None 
