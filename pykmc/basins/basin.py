...
#        ref_event = self.reference_table.table.iloc[event_idx].copy()
        ...
        result = Reconstruction(self.config, self.manager).reconstruct(supposed_initial_positions, supposed_final_positions, new_system.positions, new_system.cell, neighbors=neighbors, central_atom=central_atom)
        if not result.is_ok() :
            return result
