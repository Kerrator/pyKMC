"""State connectivity tables for basin exploration."""
...
class StatesConnectivity() :
    ...
    def save(self, outfile: str = "basin_connectivity.pickle") -> None:
        """Save the connectivity table, choosing the format from the file extension.

        ``.csv`` and ``.parquet`` are portable across pandas versions; ``.pickle``
        (the default) is faster but can fail to round-trip between mismatched pandas
        releases, so prefer ``.csv``/``.parquet`` when sharing a table for someone
        else to load.

        Parameters
        ----------
        outfile : str, optional
            Output filename. The extension (.csv, .parquet, or .pickle) selects the
            format. Default is 'basin_connectivity.pickle'.

        """
        if outfile.endswith(".csv"):
            self.df.to_csv(outfile, index=False)
        elif outfile.endswith(".parquet"):
            self.df.to_parquet(outfile, index=False)
        else:
            self.df.to_pickle(outfile)

    @classmethod
    def load(cls, infile: str) -> "StatesConnectivity":
        """Load a connectivity table saved by :meth:`save` (format from extension)."""
