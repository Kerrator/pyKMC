...
def _normalize_pbc(pbc: npt.ArrayLike | None) -> np.ndarray:
...
def atoms_of_interest_fingerprint(
    ...
    for dim in range(3):
        if pbc_array[dim] and cell_diag[dim] > 0:
            bulk_defect_diff[dim] -= (
                np.round(bulk_defect_diff[dim] / cell_diag[dim]) * cell_diag[dim]
            )
...
def _derived_coord_thr(config: Config) -> "int | None":
    ...
    """
    if (
        config.basin is not None
        and config.basin.fingerprint_coordination_thr is not None
    ):
        return config.basin.fingerprint_coordination_thr
    coord_thr = getattr(config.atomicenvironment, "coordination_threshold", None)
    if (
        config.atomicenvironment.style in ("coordination", "coordination/graph")
