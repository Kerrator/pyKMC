...
class ReferenceEventTable:
    ...
    def is_valid_new_event(
        ...
        cell : np.ndarray
            Simulation box cell.
        types : list[str] | None
            Per-atom element symbols used for full-colour graph hashing and
            per-event ``initial_types`` storage. None in grey-alloy mode.
    ...
    def _build_event_series(
        ...
        cell : np.ndarray
            Simulation box cell.
        types : list[str] | None
            Per-atom element symbols of the full system. In full-colour mode
            they colour the graph topology hashes and are stored per-event as
            ``initial_types``; None in grey-alloy mode (uncoloured, the default).
        ...
        # matching). None in grey mode / when unavailable.
...
class ActiveEventTable:
    ...
    def build_event_series(
        ...
        # An event whose per-event prefactor is not yet resolved (e.g. a basin
        # super-event built with k_prefactor=None) falls back to the k0
        # placeholder (ps^-1), the same semantic the reference table uses when no
        # nu0 is available; backfill_refined_prefactors patches the real
        # nu0-based rate afterwards for refined htst rows.
    ...
    def remove_duplicates(self, cell, neighbors_list: NeighborsList = None) -> None :
        ...
        for idx, subset in grouped:
            ...
            for jdx in subset.index:
                ...
                # atoms (finding #8). None/length-mismatched neighbours are
        ...
        # The alignment now uses each row's STORED `neighbors` ordering (the
        ...
        # scatters coordinates onto the wrong atoms for recycled rows (finding
        # #6). `neighbors_list` is retained only as the gate that enables this
        # symmetric pass.
        if neighbors_list is not None : #need neighbors list to remove symmetric duplicates
            ...
            for num_ref in symmetric_num_ref:
                ...
                for i, idx in enumerate(indices) : #Loop over indice of subset
                    ...
                    env1 = subset.loc[idx, "neighbors"] #stored ids for saddle_positions of row idx
                    ...
                    for jdx in indices[i+1:] : #to not compare two times
                        ...
                        if central_atom1 != central_atom2 : #if yes already done in part 1.
                            env2 = subset.loc[jdx, "neighbors"] #stored ids for saddle_positions of row jdx
                            if env1 is None or env2 is None : #ordering unknown -> not comparable
                                continue

                            if central_atom1 not in set(np.asarray(env2, dtype=int).tolist()) : #TODO : To check, but should not be a duplicate
                                continue
                            ...
                            if aligned is None : #no shared atoms / length mismatch -> keep both
                                continue
