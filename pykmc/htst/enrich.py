...
share the engine-agnostic math in :mod:`pykmc.rate_constant.prefactor`. Use it to
...
def lammps_forces_factory(
    ...
    positions as a non-periodic cluster (see
    :func:`pykmc.htst.profiling.build_serial_engine`), and returns
    ``(forces_fn, cell)``. Imports of ``lammps`` / the engine are deferred so the
    parser and the pure :func:`enrich_dataframe` stay importable without a
    LAMMPS build.
    """
    if len(elements) != 1:
        raise NotImplementedError(
    ...
    from .profiling import build_serial_engine

    def factory(positions: np.ndarray) -> "tuple[ForcesFn, np.ndarray]":
        engine, cell = build_serial_engine(
            positions, potential=potential, pair_style=pair_style, element=elements[0]
        )
        return engine.get_forces, cell  # type: ignore[attr-defined]
