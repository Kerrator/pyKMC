...
def _require_finite_positions(positions: "np.ndarray", op_name: str) -> None:
    ...
    ``minimize``), and the job hangs for hours. The survivor never returns from
    ``lammps_command``, so no *post-op* resync can recover it -- the error must be
    caught *before* LAMMPS runs.
