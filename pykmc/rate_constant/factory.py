...
def create_rate_constant(
    T: float, prefactor_backend_name: str, **kwargs
) -> RateConstant:
    ...
    **kwargs
    ...
    if prefactor_backend_name not in registry:
        raise ValueError(
            f"Backend '{prefactor_backend_name}' unknown. Available: {list(registry.keys())}"
        )
    return RateConstant(
        T=T, prefactor_backend=registry[prefactor_backend_name](**kwargs)
    )
