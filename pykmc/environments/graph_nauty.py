...
def graph(
    ...
    types : list[str] | None, optional
        Per-atom element symbols used to build the nauty vertex colouring
        (full-colour mode). When None (default) the graphs are uncoloured,
        reproducing the species-blind grey-alloy behaviour.
...
def make_graph(
    ...
    types : list[str] | None, optional
        Per-atom element symbols used to build the nauty vertex colouring
        (full-colour mode). When None (default) the colouring is empty, which
        is pynauty's uncoloured default (species-blind grey-alloy behaviour).
