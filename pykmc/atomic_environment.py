...
def effective_types(
    ...
    Centralises the grey/full gate so the species-blind default resolves to
    None at every call site, leaving matching species-blind.
...
class AtomicEnvironment:
    ...
    def compute_coordinationgraph(
        ...
        # Compute graph topo for all non-crystal atoms (coloured in full mode)
        list_graphs_hash = graph(neighbors_list, environment_list, non_crystal_idx, types=self.types)
        for i, idx in enumerate(non_crystal_idx):
            list_hash[idx] = list_graphs_hash[i]
