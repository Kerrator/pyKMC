...
class Worker:
    ...
    def _loop(self) -> None:
        ...
        while self._is_alive:
            ...
            if self.rank == 0:
                if r.status == DispatchResult.SILENT:
                    pass
                elif r.status == DispatchResult.SUCCESS:
                    self.world_comm.send({"type": "status", "value": {"alive": self._is_alive, "has_result": r.value is not None, "error": None}}, dest=0, tag=0)
                    if r.value is not None:
                        self.world_comm.send({"type": "result", "value": r.value}, dest=0, tag=1)
                elif r.status == DispatchResult.ERROR:
                    self.world_comm.send({"type": "status", "value": {"alive": self._is_alive, "has_result": False, "error": r.error}}, dest=0, tag=0)
    ...
    def _dispatch(self, msg: dict) -> DispatchResult:
        ...
        if op_type in self._builtins_op:
            ...
            return DispatchResult(DispatchResult.SILENT)

        handler = self.registry.get(op_type)
        if handler is None:
            return DispatchResult(DispatchResult.ERROR, error=f"Unknown operation '{op_type}'. Available: {list(self._builtins_op) + list(self.registry)}")
        ...
        self.comm.barrier()
        try:
            return DispatchResult(DispatchResult.SUCCESS, value=handler(**kwargs))
        except Exception as e:
            return DispatchResult(DispatchResult.ERROR, error=str(e))
        finally:
            self.comm.barrier()
