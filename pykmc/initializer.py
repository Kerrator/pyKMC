...
class Initializer() : 
    ...
    def initialize_system(self) -> None : 
        ...
        self.kmc.loggers.info('log', ':=> Reading initial configuration file : {}'.format(self.kmc.config.control.initial_config))
        self.kmc.system = System.create_from_file(self.kmc.config.control.initial_config)

    def initialize_engine(self) -> None : 
        """Initialize the engine based on the Config"""
        self.kmc.loggers.info('log', ':=> Initializing E/F {} Engine'.format(self.kmc.config.control.engine))
    ...
    def initialize_neighbors_list(self) -> None : 
        ...
        self.kmc.neighbors_list = NeighborsList(self.kmc.system, self.kmc.config.atomicenvironment.rnei, self.kmc.config.atomicenvironment.rcut)
    ...
    def initialize_atomic_environments(self) -> None : 
        ...
        self.kmc.atomic_environment = AtomicEnvironment(self.kmc.config.atomicenvironment.style, self.kmc.neighbors_list.neighbors_list['rnei'], self.kmc.neighbors_list.neighbors_list['rcut'], self.kmc.config.atomicenvironment.neighbors_add)
    ...
    def initialize_reference_table(self) -> None : 
        ...
        if self.kmc.config.control.reference_table is not None : 
            self.kmc.loggers.info('log', ':=> Reading Reference table file {}'.format(self.kmc.config.control.reference_table))
        else : 
    ...
    def _initialize_visited_environments(self) -> None : 
        ...
        if self.kmc.config.control.visited_environments is not None : 
            self.kmc.loggers.info('log', ':=> Initiating visited environment from file {}'.format(self.kmc.config.control.visited_environments))
            try:
        ...
        if self.kmc.config.control.visited_environments and not self.kmc.config.control.reference_table : 
            self.kmc.loggers.warning('log', "Visited environments are read from file while no reference table was provided")