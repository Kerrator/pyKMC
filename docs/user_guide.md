...
- `style = htst`: every accepted reference event gets the harmonic-transition-state-theory (Vineyard) prefactor computed from the partial Hessians of its two minima and its saddle over the free region (`free_radius` around the moving atom, centred on the saddle by default, see `free_region_center`), optionally on a cropped scratch system (`zone_radius`) and after a preminimisation (`premin`). Refined active events get their own site prefactor from the refined saddle. Under active volume the shell atoms beyond `rmov` that fall inside `free_radius` are excluded from the free region as well, since the search held them (the log reports the free-set size and the excluded shell count per request). This style needs a LAMMPS build with the `PHONON` package (the `dynamical_matrix` command): its absence is a preflight error when the simulation starts, there is no finite-difference fallback in production.
- `style = rpa`: an alias of `htst` (bare Vineyard prefactor; no recrossing correction is implemented).

With `htst`/`rpa`, `k0` is the per-event **fallback** prefactor. It is used whenever no accepted Vineyard value is available for a row: an estimate outside the acceptance window `[nu0_min_THz, nu0_max_THz]`, a geometry rejected by the kernel (not stationary within `force_tol`, unstable minimum, saddle with more than one negative mode, empty free region), or a reference table written before the current schema, which is loaded with a `WARNING` and its estimates demoted to `legacy`. The per-step `HTST prefactors:` line of the log and the `nu0`, `nu0_status` (`ok`, `rejected`, `legacy`, `pending`, `stale`) and `nu0_source` (`reference`, `site`, `k0`) columns of the event tables say which value each row used. Inside basins the exit rates use the reference prefactors; no site prefactor is computed there. A `k0` above $10^{4}$ ps$^{-1}$ is rejected for `htst`/`rpa` because it was almost certainly entered in Hz. Persistence, invalidation and recycling of the computed prefactors are described in [Physical descriptors](physical_descriptors.md); whether a site prefactor survives the next step depends on the potential's declared `interaction_range` (the pair cutoff, up to twice the cutoff for a many-body potential).

`interaction_range` is set per potential. It is the largest distance over which a held atom's position enters a free atom's rows of the partial Hessian: the pair-style cutoff for a pair potential, up to twice the cutoff for an embedded-atom, moment-tensor or three-body potential, whose energy terms couple a free atom to its second neighbours. LAMMPS does not expose the cutoff to pyKMC, so the value is the user's declaration and is never inferred. It matters only when active events are recycled between steps: a site prefactor kept from a previous step stays valid while every atom within `free_radius + interaction_range` of its moving atom (within `zone_radius` when the calculation was zone-cropped) sits within 1e-8 Å of its producing position; any relaxation inside that sphere, including the one that follows an executed event elsewhere, invalidates the row; the site is then re-refined and its prefactor recomputed at the next step as for a fresh row. At the defaults (`free_radius = 6.0`, `interaction_range = 13.0`, twice the cutoff of the Ni EAM potential shipped with the tests) the dependency sphere has a 19 Å radius, which in a cell of a few thousand atoms covers most of the system, so almost every executed event invalidates every recycled site row and the reuse buys little. Declare the range of the potential actually used (for example twice the 5.5 Å cutoff of the shipped Cu EAM potential): a matched value keeps more rows valid, a too-small one lets a moved interacting atom keep a stale spectrum. The key never enters a computed prefactor and is not stored with the table, so changing it between runs demotes nothing already catalogued.

**Active volume with `htst`/`rpa`.** Under [active volume](active_volumes.md) the search relaxes only the core within `rmov` of the event centre and holds the surrounding shell. Shell atoms that fall inside `free_radius` of a request are therefore fixed for the Hessian as well, on top of the user's `frozen_atoms`; the user constraints stay the only coordinate contract (a returned geometry is accepted or rejected on them at the engine boundary and only they invalidate recycled rows; the HTST request built from it additionally requires every fixed atom, shell included, to sit where the search left it, which holding the shell guarantees). For every request the log reports `[htst] event <key>: free set N of M atoms within free_radius R A (user-fixed excluded U, active-volume shell excluded S)`, at INFO level when S > 0 (visible at the default `verbosity = 1` of `[Control]` and above) and at DEBUG level otherwise; at `verbosity = 0` nothing reports it, and the per-step `HTST prefactors:` summary carries no shell count at any level. A reference request's active-volume core is centred on the search's central atom while its free sphere is centred on the moving atom at its saddle, so a few shell atoms usually fall inside the sphere; a site request's core and sphere share the row's atom and it often loses none. One consequence to expect: because the search relaxed only the core, the reference events it returns under active volume and the site requests of later steps can carry residual forces on core atoms above `force_tol`, and the kernel then rejects the geometry as non-stationary. Those rows run on `k0` with `nu0_status = rejected`, counted on the `HTST prefactors:` line; on a silicon test system under active volume the residual core forces observed lie between three and forty times the default `force_tol`, so most reference events and the second-step site requests use `k0` there. When a run under active volume shows a high rejected count, the options are a tighter search and relaxation convergence for the active volume, a larger `force_tol`, or accepting `k0` on those rows; pyKMC applies none of them automatically.
